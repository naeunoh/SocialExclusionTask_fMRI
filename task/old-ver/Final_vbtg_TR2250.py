...
maxTime=112.5          #max time for each throw
maxTrials=500        #max number of trials
holder=1            #player holding the ball
round=1
trialCnt=0          #total number of throws
rndCnt=0            #number of throws in each round
condition="FBALL"

instructions1 = '''
과제설명, we will test the effects of practicing mental visualization on task performance. Thus, we need you to practice your mental visualization skills. We have found that the best way to do this is to have you play an on-line ball tossing game with other participants who are logged on to the system at the same time.
...
def select_throw():
    ...
    if holder==2:
        ...
        choice=[]
        while len(choice)==0 or choice [0] not in ('1','2'):
            core.wait(0.01)
